...
class density_matrix:
    ...
    initial = {"phy_plus":[0, 0.3], "phy_minus":[1, 0.2], "psi_plus":[2, 0.2], "psi_minus":[3, 0.3]} #latter one in [0, 1](1 is latter one) is the rate of the state.
    I = np.array(((1, 0), (0, 1)))
    x = np.array(((0, 1), (1, 0)))
    ...
    def two_tomography(self):
        ...
        for i in initial.values():#iterate for each bell pair
            ...
            for measure in measurement_basis:
                    prob = np.vdot(np.dot(con(bell[i[0]]), con(measure)), np.dot(measure, bell[i[0]]))  #con is the conjugate operation
    ...
    def tomograph_density(self, iter):
        ...
        for j in initial.values():
            ...
            for measure in measurement_basis:
                    prob = np.vdot(np.dot(con(bell[j[0]]), con(measure)), np.dot(measure, bell[j[0]]))  #con is the conjugate operation
            ...
            for c in range(0, 5, 2):
                st.append(result[c][c] - result[c][c + 1] + result[c + 1][c] - result[c + 1][c + 1])

            for col in range(0, 5, 2):
                for low in range(0, 5, 2):
                    # print(col, low)
                    # print(re[col][low] + re[col + 1][low + 1] + re[col][low + 1] + re[col + 1][low])
                    st.append(result[col][low] + result[col + 1][low + 1] - result[col][low + 1] - result[col + 1][low])
...
if __name__ == '__main__':
    density_matrix.iterate_sim(10000) #difine how many times iterate in the method iterate_sim(iteration number)
    # density_matrix.evaluation_density(10000)
